...
function sudokuTiempoReal(imagen,plantillas,MOSTRAR)
%% Modo de ejecución
MOSTRAR;
...
if MOSTRAR
    hold on
    recuadro = plot(posx+[0 resol resol 0 0],posy+[0 0 resol resol 0],'g');
    hold off
end
%% Recortar imagen
...
if MOSTRAR
    imshow(imBin3);
    hold on;
    plot(verticesOriginales(:,1),verticesOriginales(:,2),'r+',...
        'Linewidth',15)
end
...
T_original2ortogonal = fitgeotrans(verticesOriginales,...
    res*[0 0;0 1;1 1;1 0],'projective');
...
if MOSTRAR
    for i=numel(objetosOrtogonal):-1:1
        rectangle('Position',objetosOrtogonal(i).BoundingBox,'EdgeColor'...
            ,'green')
    end
end
...
if ~isempty(solucion)
    ...
    for i=1:9
        for j=1:9
            % El eje y está invertido con respecto a las casillas 
            % => posY = -j
            posX = i-0.6;
            posY = -j+0.5;
    ...
    % 2.1. Se crea un recuadro que se usará como referencia para la
    % posición
    ...
    imSintetica = imcrop(imSintetica,...
        [verticesSinteticos(1,1),verticesSinteticos(1,2),...
    ...
    if MOSTRAR
        hold on
        plot(verticesSinteticos(:,1),verticesSinteticos(:,2),'r+',...
            'Linewidth',15)
        figure
        imshow(imSintetica)
    end
    ...
    objetosSinteticos = regionprops(~imSintetica,'FilledArea','PixelList');
    [verticesSinteticos,~] = detectarPerimetroSudoku(objetosSinteticos);
    % 2. Se genera la transformacion del sudoku a la imagen captada
    % haciendo coincidir directamente las esquinas del sudoku_sintetico con
    % las de la imagen del sudoku_original. De esta forma la superposición
    % es directa.
    for i=1:4
        verticesOriginales(i,1) = verticesOriginales(i,1)+posx;
        verticesOriginales(i,2) = verticesOriginales(i,2)+posy;
    end
    T_sintetic2original = cp2tform(verticesSinteticos,verticesOriginales...
        ,'projective');
    imSintetica = imtransform(~imSintetica,T_sintetic2original,'XData',...
        [1 size(im,2)], 'YData',[1 size(im,1)]);
    
    i_gray = rgb2gray(im);
    imSintetica = uint8(~imSintetica);
    imFinal = i_gray .* imSintetica;
